...
产品承载的主要行为是连续记录。系统在保留原始上下文的基础上，提取日期、时间、项目归属和待催事项，再生成今日、本周、未来和历史视图。
...
早期需求更接近 Todo List 或 DDL 提醒器，关注如何创建和管理任务。实际使用表明，工作信息通常先以邮件、聊天、会议和临时安排的形式出现。用户首先需要保存完整内容，随后才会从中识别行动项。
...
网页原型已经具备基础功能，但每次使用仍要先打开浏览器并找到对应页面，使用阻力与进入飞书后再寻找工具入口相近。WPS 即使固定在任务栏，也需要从多个工作簿中再打开专门的任务表，无法形成一个点击即用的独立入口。原本固定在任务栏的便签因此仍然更顺手。
...
早期迭代中曾出现页面更新异常，本地记录未备份，只能重新从微信、便签和邮件中恢复内容。
...
V1 使用本地规则处理明确日期、时间、项目上下文、`>` 待催和 `//` 备注。

下一阶段计划保留已经验证的规则能力，让 AI 处理模糊时间、非标准表达和复杂长文本。周报、催办文本和延期说明等内容生成能力建立在可靠任务数据之上，分阶段推进。
...
真实工作主要按日期推进。多个项目可能出现同名交付物，例如“保真文档”或“总结报告”，因此系统支持：
...
后续任务继承项目上下文，并在右侧显示 `[项目名称]` 前缀。日期继续承担排序和提醒作用。
...
原文包含计划、背景、备注和业务判断。任务列表由原文派生，完成状态属于用户对任务的处理结果。
...
任务需要承载负责人、阶段历史、多次催办记录等长期状态时，再考虑独立任务实体和更稳定的任务标识。

### 决策四：采用 PWA、LocalStorage 和手动备份

**原因**

V1 首先服务个人桌面工作流。PWA 提供独立入口、任务栏固定、离线打开和较低的开发维护成本。LocalStorage 可以在无账号、无后端的情况下保存原文、解析结果和任务状态。
...
```mermaid
flowchart LR
    A[本周随记 / 长线备忘] --> B[LocalStorage 自动保存]
    A --> C[本地日期与任务规则解析]
    C --> D[派生任务与缓存]
    D --> E[今日 / 本周 / 未来 / 历史视图]
...
日常迭代采用小步修改，重点测试本次新增能力。V1.0 发布前建立完整的 `TESTING.md`，逐项执行输入保存、日期解析、上下文继承、状态历史、待催、提醒、备份恢复、PWA、跨周和异常场景测试。系统性回归全部通过，未发现新的阻断问题。

## 6. 当前架构边界

V1.0 当前支持个人、本地和桌面为主的工作流。以下能力留在后续阶段评估：
...
下一阶段的重点是先解决启动性能和外部用户输入门槛，再通过真实使用数据决定 AI、云端和协作能力的投入顺序。
