...
本文记录 DDL Assistant V1 在产品定位、范围控制和实现方式上的关键选择。重点说明每项决定解决了什么问题、接受了哪些代价，以及未来在什么条件下重新评估。
...
v1.1 解决首次使用和窄窗口常驻使用中的摩擦，但不改成完整新手教程或任务管理系统：

- 空状态解释“带日期的事项会进入提醒”；
- 主输入框 placeholder 展示少量常用写法；
- 记录区和提醒区保持单列，使用记录区约 55%、提醒区约 45% 的固定比例；
- 已完成事项复制服务于周报和沟通记录。

这些能力均基于既有原文、派生任务和完成状态，不新增独立任务数据模型。
...
flowchart LR
    ...
    C --> D[派生任务与缓存]
    D --> E[今日 / 本周 / 未来 / 历史视图]
    E --> F[完成状态与 completedAt 标记时间戳独立保存]
    E --> G[点击任务定位原文]
    E --> J[单条与按周复制已完成事项]
    B --> H[JSON 手动备份与校验恢复]
...
- 任务完成状态保存在独立的状态对象中；
- 勾选时记录 `completedAt`，其含义是用户点击勾选框时的标记完成时间戳，不是经过核验的实际完成时间；
- 当天完成的任务保留在今日列表底部；
- 早于今天完成的任务进入历史，并按本周、上周和更早周次分组；
- 周随记归档不会删除未完成任务。

按周复制已完成事项时，不使用 `completedAt` 判断所属周。普通任务使用 `dueDate`，待跟进任务使用 `followUpDate`，这样可以避免延迟勾选导致周报归属错误。
...
v1.1 使用固定的上下布局，记录区约 55%、提醒区约 45%，两区均支持内部滚动。记录区头部保持紧凑，主输入框、长线事项和底部工具位于记录区内部滚动容器中，长线事项展开时不再压缩主输入框。旧版本若曾在浏览器中留下布局比例键，新版本会忽略该值，不读取、不写入、不导入。
...
V1 当前支持本地和桌面为主的工作流。以下能力留在后续阶段评估：
